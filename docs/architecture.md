...
- **Per-request account ownership**: `AccountGuard.owned(accountId)`, called
  at the top of every account-scoped controller method (portfolio,
  scorecard, orders, blotter, settlements) — a valid JWT alone isn't enough,
  the token's user must actually own the account being queried.

## Persistence

Flyway (`backend/src/main/resources/db/migration`, `V1`–`V5`) owns the
